...
const helpers = {
  ...
  formatDate(date: dayjs.ConfigType | undefined, format: string): string {
  ...
  diffTimeByNow(time: dayjs.ConfigType | undefined): diffTime {
...
const install = (app: App<Element>): App<Element> => {
  app.config.globalProperties.$h = helpers;
  return app;
};

const combineWithAttributesOn = function (
...
export { install as default, helpers as helper, combineWithAttributesOn };
