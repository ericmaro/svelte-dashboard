<template lang="jsx">
    <div class="flex flex-no-wrap bg-gray-50 min-h-full font-rubik w-full">
        <!-- Sidebar starts -->
        <!-- Remove class [ hidden ] and replace [ sm:flex ] with [ flex ] -->
        <div class="w-1/5 xl:w-1/6 2xl:w-64 absolute sm:relative bg-green-600 text-gray-200 tshadow flex-col justify-between hidden md:flex">
            <div class="space-y-10">
                <div class="w-full bg-green-700 text-white px-5 py-4">
                    <!-- Start Logo -->
                    <a class="flex items-center xl:space-x-2">
                      <img
                      ...
                      <span class="hidden xl:block text-lg">
                        Ubia<span class="font-medium">soko</span>
                      </span>
                    </a>
                    <!-- End Start Logo -->
                </div>
                <ul class="px-3">
                    <li v-for="(item, index) in menuItems" :key="index" class="w-full cursor-pointer">
                        <div 
                            @click="onClickParentRoute(item.route)" 
                            class="flex w-full justify-between hover:text-white items-center rounded-md p-3" 
                            :class="[route.name == item.route? 'bg-green-700 text-white' : '']"
                            >
                            <!-- v-if="index == 0 || checkPermission(user, item.route)" -->
                            <div  class="flex items-center space-x-2">
                                <span v-html="item.icon"></span>
                                <span class="text-sm">{{item.title}}</span>
                            </div>
                          <div v-if="item.subMenu && item.subMenu.length > 0" class="flex items-center justify-center text-xs">
                            <ChevronDownIcon />
                          </div>
                        </div>
                        ...
                        >
                            <div v-if="activeDropdown == item.route && item.subMenu" class="ml-10 mt-2">
                                <ul v-if="item.subMenu && item.subMenu.length > 0">
                                    <li 
                                        class="text-sm hover:text-white p-2 rounded-md" 
                                        :class="[route.name == subMenu.route? 'bg-green-700 text-white' : '']"  
                                        v-for="(subMenu, subMenuKey) in item.subMenu" :key="subMenuKey"  
            ...
            <div class="px-8 border-t border-green-700">
                <ul class="w-full flex items-center justify-between ">
                    ...
                    <li class="cursor-pointer text-white pt-5 pb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="ih-6 w-6" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z"></path>
        ...
        <div class="w-64 z-40 absolute bg-gray-800 shadow md:h-full flex-col justify-between sm:hidden transition duration-150 ease-in-out" id="mobile-nav">
            <div id="openSideBar" class="h-10 w-10 bg-gray-800 absolute right-0 mt-16 -mr-10 flex items-center shadow rounded-tr rounded-br justify-center cursor-pointer" @click="sidebarHandler(true)">
            ...
            <div id="closeSideBar" class="h-10 w-10 hidden bg-gray-800 absolute right-0 mt-16 -mr-10 items-center shadow rounded-tr rounded-br justify-center cursor-pointer text-white" @click="sidebarHandler(false)">
        ...
        <div class="container mx-auto py-0 px-6 w-4/5 xl:w-5/6">
            <!-- Remove class [ border-dashed border-2 border-gray-300 ] to remove dotted border -->
            <div class="w-full h-screen border-gray-300 space-y-4">
...
export default defineComponent({
  ...
  setup() {
    ...
    function onClickParentRoute(routeName: string): void {
      router.push({ name: routeName });
      activeDropdown.value = routeName;
    }

    onMounted(() => store.dispatch("users/getLoggedInUserAction"));

    function checkPermission(user: User, routeName: string): boolean {
      if (user) {
        const status = user.permissions.some(
          (el: Permission) => el.name === "view " + routeName
        );
        return status;
      } else {
        return false;
